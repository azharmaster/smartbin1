    ...
    <div class="container-fluid">
        ...
        <div class="row mb-4 align-items-center no-print">
                ...
                <form method="GET" action="{{ route('collection-trips.summary') }}" class="row g-2 align-items-end">
                    ...
                    @if ($period === 'daily')
                        <div class="col-md-3">
                            ...
                            <input type="date" name="date" value="{{ $dateInput }}" class="form-control fw-bold"
                    ...
                    @if ($period === 'weekly')
                        <div class="col-md-3">
                            ...
                            <input type="week" name="week" value="{{ $weekInput }}" class="form-control fw-bold"
                                onchange="this.form.submit()">
                        </div>
                    @endif
                    ...
                    @if ($period === 'monthly')
                        <div class="col-md-3">
                            ...
                            <input type="month" name="month" value="{{ $monthInput }}" class="form-control fw-bold"
                                onchange="this.form.submit()">
                        </div>
                    @endif
            ...
            <div class="col-lg-2 ms-auto text-end">
                <a href="{{ route('collection-trips.index', ['asset_id' => $assetId]) }}"
        ...
        <div class="row g-4 mb-4 no-print">
            @php
                ...
                $avgPerBin = $activeBins > 0 ? round($totalTrips / $activeBins, 1) : 0;
                $topWeekdays = collect($weekdayLabels)
                    ->zip($weekdayData)
                    ->map(fn($pair) => ['label' => $pair[0], 'count' => $pair[1]])
                    ->reject(fn($item) => $item['count'] == 0)
                    ->sortByDesc('count')
                    ->take(3)
                    ->values();
                $topBins = collect($mostUsedBinLabels)
                    ->zip($mostUsedBinData)
                    ->map(fn($pair) => ['label' => $pair[0], 'count' => $pair[1]])
                    ->sortByDesc('count')
                    ->take(3)
                    ->values();
                $topCapacities = collect($compartmentCapacityLabels)
                    ->zip($compartmentCapacityData)
                    ->map(fn($pair) => ['label' => $pair[0], 'value' => $pair[1]])
                    ->sortByDesc('value')
                    ->take(3)
                    ->values();
            @endphp
                            ...
                            <div class="flex-grow-1">
                                ...
                                <div class="metric-info mt-2">
                                    <div class="d-flex justify-content-between">
                                        <span class="text-white-50">Active Bins</span>
                                        <span class="fw-semibold">{{ $activeBins }}</span>
                                    </div>
                                    <div class="d-flex justify-content-between">
                                        <span class="text-white-50">Avg / Bin</span>
                                        <span class="fw-semibold">{{ $avgPerBin }}</span>
                                    </div>
                                </div>
                            </div>
                            <div class="text-white-50 ms-2">
                    ...
                    <div class="card-body text-white">
                        <div class="d-flex justify-content-between align-items-start">
                            <div class="flex-grow-1">
                                ...
                                <h2 class="mb-0 fw-bold">{{ $averageTripsMetric['value'] }} <small
                                        class="fs-6">/{{ $averageTripsMetric['unit'] }}</small></h2>
                                <div class="metric-info mt-2">
                                    <div class="d-flex justify-content-between">
                                        <span class="text-white-50">Total Trips</span>
                                        <span class="fw-semibold">{{ number_format($totalTrips) }}</span>
                                    </div>
                                    <div class="d-flex justify-content-between">
                                        <span class="text-white-50">Range</span>
                    ...
                    <div class="card-body text-white">
                        <div class="d-flex justify-content-between align-items-start">
                            <div class="flex-grow-1">
                                ...
                                <h2 class="mb-0 fw-bold" style="font-size: 1.25rem;">{{ $weekdayPeakLabel }}</h2>
                                @if($topWeekdays->isNotEmpty())
                                    <div class="metric-info mt-2">
                                        @foreach($topWeekdays as $wd)
                                            <div class="d-flex justify-content-between">
                                                <span class="text-white-50 text-truncate">{{ \Illuminate\Support\Str::limit($wd['label'], 3, '') }}</span>
                                                <span class="fw-semibold">{{ $wd['count'] }}</span>
                                            </div>
                                        @endforeach
                                    </div>
                                @else
                                    <small class="d-block mt-2 text-white-50">No data for this period.</small>
                                @endif
                            </div>
                            <div class="text-white-50 ms-2">
                    ...
                    <div class="card-body text-white">
                        <div class="d-flex justify-content-between align-items-start">
                            <div class="flex-grow-1">
                                ...
                                <h2 class="mb-0 fw-bold" style="font-size: 1.05rem;">{{ $mostUsedBin }}</h2>
                                @if($topBins->count() > 1)
                                    <div class="metric-info mt-2">
                                        @foreach($topBins->slice(1, 2) as $bin)
                                            <div class="d-flex justify-content-between">
                                                <span class="text-white-50 text-truncate" style="max-width: 70%;">{{ $bin['label'] }}</span>
                                                <span class="fw-semibold">{{ $bin['count'] }}</span>
                                            </div>
                                        @endforeach
                                    </div>
                                @else
                                    <small class="d-block mt-2 text-white-50">Only one bin recorded.</small>
                                @endif
                            </div>
                            <div class="text-white-50 ms-2">
                    ...
                    <div class="card-body text-white">
                        <div class="d-flex justify-content-between align-items-start">
                            <div class="flex-grow-1">
                                ...
                                <h2 class="mb-0 fw-bold" style="font-size: 1.05rem;">{{ $highestCapacityTile['value'] }}
                                    {{ $highestCapacityTile['label'] }}</h2>
                                @if($topCapacities->count() > 1)
                                    <div class="metric-info mt-2">
                                        @foreach($topCapacities->slice(1, 2) as $cap)
                                            <div class="d-flex justify-content-between">
                                                <span class="text-white-50 text-truncate" style="max-width: 65%;">{{ $cap['label'] }}</span>
                                                <span class="fw-semibold">{{ $cap['value'] }}%</span>
                                            </div>
                                        @endforeach
                                    </div>
                                @else
